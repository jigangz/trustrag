import React, { useState } from "react";
import { Send, Loader2, HelpCircle, Wifi, WifiOff } from "lucide-react";
import { useWebSocketQuery } from "../hooks/useWebSocketQuery";
import StreamingAnswer from "./StreamingAnswer";
import StatusIndicator from "./StatusIndicator";
import CancelButton from "./CancelButton";
import ConfidenceBadge from "./ConfidenceBadge";
import SourceCard from "./SourceCard";
import ConsistencyView from "./ConsistencyView";

const EXAMPLES = [
  "What are the structural load requirements for floor slabs?",
  "What safety measures are required for scaffolding?",
  "What are the concrete curing time specifications?",
];

export default function StreamingQueryPanel() {
  const [question, setQuestion] = useState("");
  const {
    connectionStatus,
    queryStatus,
    answer,
    sources,
    trust,
    consistency,
    error,
    sendQuery,
    cancelQuery,
  } = useWebSocketQuery();

  const isActive = ["retrieving", "generating", "verifying_trust"].includes(
    queryStatus
  );

  const handleSubmit = (e) => {
    e.preventDefault();
    const q = question.trim();
    if (!q || isActive) return;
    sendQuery(q);
  };

  const askExample = (q) => {
    setQuestion(q);
    sendQuery(q);
  };

  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      {/* Connection status */}
      {connectionStatus !== "connected" && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-500/10 border border-amber-500/30 text-xs text-amber-400">
          <WifiOff className="w-3.5 h-3.5" />
          Connecting to server...
        </div>
      )}

      {/* Query input */}
      <form onSubmit={handleSubmit} className="relative">
        <input
          type="text"
          value={question}
          onChange={(e) => setQuestion(e.target.value)}
          placeholder="Ask a question about your construction documents..."
          disabled={isActive || connectionStatus !== "connected"}
          className="w-full bg-slate-900 border border-slate-700 rounded-xl py-4 pl-5 pr-14 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/25 transition-all disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={isActive || !question.trim() || connectionStatus !== "connected"}
          className="absolute right-2 top-1/2 -translate-y-1/2 p-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 disabled:text-slate-500 text-white transition-colors"
        >
          {isActive ? (
            <Loader2 className="w-4 h-4 animate-spin" />
          ) : (
            <Send className="w-4 h-4" />
          )}
        </button>
      </form>

      {/* Status + Cancel row */}
      {isActive && (
        <div className="flex items-center justify-between">
          <StatusIndicator stage={queryStatus} />
          <CancelButton queryStatus={queryStatus} onCancel={cancelQuery} />
        </div>
      )}

      {/* Example questions */}
      {queryStatus === "idle" && (
        <div className="animate-fade-in">
          <div className="flex items-center gap-2 mb-3">
            <HelpCircle className="w-4 h-4 text-slate-600" />
            <span className="text-xs text-slate-600">Example questions</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {EXAMPLES.map((q) => (
              <button
                key={q}
                onClick={() => askExample(q)}
                className="text-xs px-3 py-2 rounded-lg border border-slate-800 text-slate-400 hover:border-slate-600 hover:text-slate-300 transition-colors"
              >
                {q}
              </button>
            ))}
          </div>
        </div>
      )}

      {/* Error */}
      {error && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 text-sm text-red-400 animate-fade-in">
          <p className="font-medium">{error.code}</p>
          <p className="text-xs mt-1 text-red-400/80">{error.message}</p>
        </div>
      )}

      {/* Trust badge - verifying state */}
      {queryStatus === "verifying_trust" && !trust && (
        <ConfidenceBadge verifying />
      )}

      {/* Trust badge - final */}
      {trust && (
        <ConfidenceBadge score={trust.score} breakdown={trust.breakdown} />
      )}

      {/* Streaming answer */}
      <StreamingAnswer answer={answer} queryStatus={queryStatus} />

      {/* Sources */}
      {sources?.length > 0 && (
        <div className="animate-fade-in">
          <h3 className="text-sm font-semibold text-slate-400 mb-3">
            Sources ({sources.length})
          </h3>
          <div className="grid gap-3 md:grid-cols-2">
            {sources.map((src, i) => (
              <SourceCard key={i} source={src} />
            ))}
          </div>
        </div>
      )}

      {/* Consistency */}
      {consistency && <ConsistencyView data={consistency} />}

      {/* Cancelled state */}
      {queryStatus === "cancelled" && (
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 text-xs text-slate-500 animate-fade-in">
          Query cancelled. Partial answer shown above.
        </div>
      )}
    </div>
  );
}
